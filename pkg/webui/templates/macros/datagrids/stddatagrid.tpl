    ...
    WebUI.loadScriptCdn("{{.cdn_url}}","datatables/datatables.min.js",function(){
      ...
      $("#tblDataGrid_{{.id}}_length").prepend('<div class="dataTables_custombtns mb-2"><button id="btnReload_{{.id}}" class="btn btn-sm border px-3" title="Reload"><i class="fa fas fa-fw fa-refresh fa-sync-alt"></i></button>'+
        {{if .export.types}}
          ...
          {{if InStrSlice .export.types "csv"}}'<button id="btnExpCSV_{{.id}}" class="dropdown-item pl-3 pr-4 py-1"><i class="fa fas fa-fw fa-file-text-o fa-file-csv"></i> csv</button>'+{{end}}
          {{if InStrSlice .export.types "xls"}}'<button id="btnExpXLS_{{.id}}" class="dropdown-item pl-3 pr-4 py-1"><i class="fa fas fa-fw fa-file-excel-o fa-file-excel"></i> excel</button>'+{{end}}
          {{if InStrSlice .export.types "print"}}'<button id="btnPRINT_{{.id}}" class="dropdown-item pl-3 pr-4 py-1"><i class="fa fas fa-fw fa-print"></i> print</button>'+{{end}}
          '</div></div>'+
        {{end}}
