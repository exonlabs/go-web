...
func (s *Server) handleRequest(
	...
	defer func() {
		if err := recover(); err != nil {
			...
			s.Logger.Trace1(string(debug.Stack()))
		}
	}()
	...
	ctx := &Context{
		Server:  s,
		Logger:  s.Logger.NewChildLogger(name),
...
func (s *Server) Start(host string, port int) error {
	// adjust logging
	if s.Logger == nil {
		s.Logger = xlog.GetLogger()
	}
	if s.Reqlogger == nil {
		s.Reqlogger = xlog.NewLogger("reqlogger")
	}
	s.Reqlogger.SetFormatter(xlog.NewStdFormatter(
		"{message}", "2006-01-02 15:04:05.000000"))
